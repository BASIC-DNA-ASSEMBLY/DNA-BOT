...
metadata = {
    'protocolName': 'Media_Bot: Media Assembly and Subculturing (OT-2)',
    'author': 'Liam Hallett, Shishun Liang',
    'description': 'Automated media assembly and subculture from preculture plate based on CSV input for Opentrons OT-2',
    'apiLevel': '2.10'
}
...
DISPENSE_RATE = 0.4     # Rate for dispensing (0.4 = 40% speed)
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    # Load tipracks - 1000ul for media transfers, 300ul for subculture transfers
    media_tips = protocol.load_labware('opentrons_96_tiprack_1000ul', '11', 'Media Tips')
    transfer_tips = protocol.load_labware('opentrons_96_tiprack_300ul', '4', 'Transfer Tips')
    
    # Load OT2 pipettes
    multi_pipette = protocol.load_instrument('p300_multi_gen2', mount='right', tip_racks=[transfer_tips])
    single_pipette = protocol.load_instrument('p1000_single_gen2', mount='left', tip_racks=[media_tips, transfer_tips])

    mix_plate = protocol.load_labware('corning_96_wellplate_360ul_flat', '5', 'Mix Plate')
    preculture_plate = protocol.load_labware('corning_96_wellplate_360ul_flat', '6', 'Preculture Plate')
    ...
    for col in unique_columns:
        ...
        for plate in final_plates:
            ...
            # Transfer 10ul from mix plate to final plate
