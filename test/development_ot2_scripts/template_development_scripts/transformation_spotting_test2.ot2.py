...
spotting_dicts = [{'A1': 5, 'B1': 5, 'C1': 5, 'D1': 5, 'E1': 5, 'F1': 5, 'G1': 5}, {'A2': 5, 'B2': 5, 'C2': 5, 'D2': 5, 'E2':5}, {'A1': 1, 'B1': 1, 'C1': 1, 'D1': 1, 'E1': 1, 'F1': 1, 'G1': 1}]


def sample_num(spotting_dicts):
    """Determines sample number from spotting_dicts input.
    ...
    """
    return sum([len(element) for element in spotting_dicts])
...
def tiprack_slots(
    ...
    """
    total_tips = sample_num * tips_per_sample
    tiprack_slots = total_tips // 96 + 1 if total_tips % 96 > 0 else 0
    return tiprack_slots


def spotting_wells(spotting_dicts):
    """Evaluates spotting_dicts and returns wells used during transformation and spotting.
    ...
    for dictionary in spotting_dicts:
        for well, _ in dictionary.items():
            wells.append(well)
    unique_wells = [well for i, well in enumerate(
...
def spot_transformations(
    ...
    for x in range(len(transformation_cols)):
        ...
        for well, vol in spotting_dicts[x].items():
            spot(transformation_plate.wells(well),
                 agar_trays['1'].wells(well), vol)
        if agar_tray_num == 2:
            for well, vol in spotting_dicts[x].items():
                spot(transformation_plate.wells(well),
                     agar_trays['2'].wells(well), vol)
...
# Global variables
agar_tray_num = 1
sample_num = sample_num(spotting_dicts)
...
p10_slots = CANDIDATE_P10_SLOTS[
    :tiprack_slots(sample_num, p10_tips_per_sample)]
p300_slots = CANDIDATE_P300_SLOTS[
    :tiprack_slots(sample_num, P300_TIPS_PER_SAMPLE)]
...
# Run functions
transformation_setup(spotting_wells(spotting_dicts))
phase_switch()
...
for c in robot.commands():
    print(c)
# python transformation_spotting_test2.ot2.py
