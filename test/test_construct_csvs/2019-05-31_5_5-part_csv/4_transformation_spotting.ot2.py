...
def tiprack_slots(spotting_tuples):
    ...
    p300_tiprack_slots = p300_tips // 96 + 1 if p300_tips % 96 > 0 else p300_tips / 96
...
def spot_transformations(
    ...
    max_spot_vol (float): Maximum volume that is spotted for each time. 
    ...
    def spot_tuple(
        ...
        """Spots all reactions defined by the spotting tuple. Requires the function spot.
            ...
            tray_num (str): Agar tray used during spotting.
        ...
        while max(spot_vols) > 0:
                for index, spot_vol in enumerate(spot_vols):
                    if spot_vol == 0:
                        pass
                    elif spot_vol <= max_spot_vol:
                        spot(transformation_plate.wells(source_wells[index]),
                        agar_trays[tray_num].wells(target_wells[index]), spot_vol)
                        spot_vols[index] = spot_vols[index] - spot_vol
                    else:
                        spot(transformation_plate.wells(source_wells[index]),
                        agar_trays[tray_num].wells(target_wells[index]), max_spot_vol)
                        spot_vols[index] = spot_vols[index] - max_spot_vol
    ...
    for spotting_tuple in spotting_tuples:
        ...
        spot_tuple(spotting_tuple, tray_num="1")
        if agar_tray_num == 2:
            spot_tuple((spotting_tuple[0], spotting_tuple[1],
             tuple([tray_2_vol]*len(spotting_tuple[2]))), tray_num="2")
...
# Define labware
p10_tipracks = [labware.load('opentrons-tiprack-10ul', slot) for slot in p10_slots]
...
spot_transformations(spotting_tuples, agar_tray_num=agar_tray_num)
