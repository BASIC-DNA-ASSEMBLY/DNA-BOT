...
__LABWARES={"OT-2_tiprack_20ul": {"id": "opentrons_96_tiprack_20ul"}, "OT-2_tiprack_300ul": {"id": "opentrons_96_tiprack_300ul"}, "24_tuberack_1500ul": {"id": "e14151500starlab_24_tuberack_1500ul"}, "clip_source_plate": {"id": "nest_96_wellplate_100ul_pcr_full_skirt"}, "clip_plate": {"id": "nest_96_wellplate_100ul_pcr_full_skirt"}, "mix_plate": {"id": "nest_96_wellplate_100ul_pcr_full_skirt"}, "final_assembly_plate": {"id": "nest_96_wellplate_100ul_pcr_full_skirt"}, "transform_plate": {"id": "nest_96_wellplate_100ul_pcr_full_skirt"}, "agar_plate": {"id": "nest_96_wellplate_100ul_pcr_full_skirt"}, "12_reservoir_21000ul": {"id": "nest_12_reservoir_15ml"}, "96_deepwellplate_2ml": {"id": "nest_96_wellplate_2ml_deep"}, "12_corning_wellplate": {"id": "corning_12_wellplate_6.9ml_flat"}}
...
__LABWARES={
    ...
    "transformation_plate": {"id": "nest_96_wellplate_100ul_pcr_full_skirt"}, 
    "transformation_plate_wo_thermo": {"id": "nest_96_wellplate_100ul_pcr_full_skirt"}, 
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    # p20_slots = CANDIDATE_SINGLE_SLOTS[:p20_p300_tiprack_slots[0]]
    # p300_slots = CANDIDATE_MULTI_SLOTS[:p20_p300_tiprack_slots[1]]
    
    single_tipracks = [protocol.load_labware(SINGLE_TIPRACK_TYPE, slot) for slot in p20_slots]
        # changed to protocol.load_labware for API version 2
    multi_tipracks = [protocol.load_labware(MULTI_TIPRACK_TYPE, slot) for slot in p300_slots]

    INITIAL_TIP = 'A1'
    # Constants
    #PIPETTE_TYPE = __LABWARES['p300_multi']['id']
    MULTI_PIPETTE_TYPE = __HARDWARE['multi_pipette']['id']
    MULTI_PIPETTE_MOUNT = __HARDWARE['multi_pipette_mount']['id']
    SINGLE_PIPETTE_TYPE = __HARDWARE['single_pipette']['id']
    SINGLE_PIPETTE_MOUNT = __HARDWARE['single_pipette_mount']['id']
        ### Loading Pipettes
    multi_pipette = protocol.load_instrument(MULTI_PIPETTE_TYPE, mount=MULTI_PIPETTE_MOUNT,tip_racks=multi_tipracks)
    ...
    def outgrowth(
        ...
        multi_pipette.flow_rate.aspirate = SOC_ASPIRATION_RATE
        multi_pipette.transfer(SOC_VOL, soc, transformation_cols,
                              new_tip='always', mix_after=SOC_MIX_SETTINGS)
        multi_pipette.flow_rate.aspirate = P300_DEFAULT_ASPIRATION_RATE
    ...
    def spot_transformations(
        ...
        spotting_dispense_rate (float): Rate single_pipette dispenses at during spotting.
        stabbing_depth (float): Depth single_pipette moves into agar during spotting.
        max_spot_vol (float): Maximum volume that is spotted per spot reaction.
        ...
        def spot(
            ...
            single_pipette.aspirate(spot_vol + dead_vol, source[0])
            # old code:
                # single_pipette.aspirate(spot_vol + dead_vol, source)
            ...
            single_pipette.move_to(target[0].top(SAFE_HEIGHT))
            single_pipette.move_to(target[0].top(DISPENSING_HEIGHT))
            # old code:
                # single_pipette.move_to(target.top(SAFE_HEIGHT))
                # single_pipette.move_to(target.top(DISPENSING_HEIGHT))
            ...
            single_pipette.move_to(target[0].top(-1 * stabbing_depth))
            # old code:
                # single_pipette.move_to(target.top(-1*stabbing_depth))
        ...
        for spotting_tuple in spotting_tuples:
            ...
            for col in unique_cols:
                ...
                multi_pipette.mix(TRANSFORMATION_MIX_SETTINGS[0], TRANSFORMATION_MIX_SETTINGS[1],transformation_plate.columns_by_name()[col][0])
    ...
    # Define labware
    # p20_tipracks = [protocol.load_labware(SINGLE_TIPRACK_TYPE, slot) for slot in p20_slots]
    #     # changed to protocol.load_labware for API version 2
    # p300_tipracks = [protocol.load_labware(MULTI_TIPRACK_TYPE, slot) for slot in p300_slots]
        # changed to protocol.load_labware for API version 2
    # single_pipette = protocol.load_instrument(__LABWARES['p20_single']['id'], P20_MOUNT, tip_racks=p20_tipracks)
    #     # changed to protocol.load_instrument for API version 2
    # multi_pipette = protocol.load_instrument(__LABWARES['p300_multi']['id'], P300_MOUNT, tip_racks=p300_tipracks)
    #     # changed to protocol.load_instrument for API version 2
